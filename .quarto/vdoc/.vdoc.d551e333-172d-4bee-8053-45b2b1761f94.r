...
team_code <- function(city, team) {
  case_when(
    ...
    city == "Washington" & team == "Bullets" ~ "WSH",
    city == "Washington" & team == "Wizards" ~ "WSH",
    TRUE ~ str_to_upper(str_sub(city, 1, 3))
  )
}
...
unique_scores_df <- games %>%
  mutate(
    ...
    result = if_else(homeScore > awayScore, "Home win", "Away win")
  ) %>%
...
plot <- ggplot(unique_scores_df, aes(x = awayScore, y = homeScore, color = total_points)) +
  ...
  geom_text(aes(label = label), size = 2.5, color = "black", check_overlap = TRUE) +
  scale_x_continuous(breaks = seq(0, max_score + 10, by = 10), expand = expansion(mult = c(0.02, 0.05))) +
  scale_y_continuous(breaks = seq(0, max_score + 10, by = 10), expand = expansion(mult = c(0.02, 0.05))) +
